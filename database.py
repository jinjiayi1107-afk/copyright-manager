...
def get_records(table, conditions=None, order_by=None):
    """
    获取记录列表
    参数：table - 表名, conditions - WHERE条件, order_by - 排序
    返回：记录列表（字典形式）
    """
    conn = get_db_connection()
    if not conn:
...
def count_records(table, conditions=None):
    """
    统计记录数量
    参数：table - 表名, conditions - WHERE条件
    返回：记录数量
    """
    conn = get_db_connection()
    if not conn:
...
def migrate_from_sqlite(sqlite_path):
    """
    从SQLite迁移数据到MySQL
    参数：sqlite_path - SQLite数据库文件路径
    """
    import sqlite3
    
    print(f"[迁移] 开始从SQLite迁移数据: {sqlite_path}")
    ...
    try:
        ...
        print(f"[迁移错误] 无法连接SQLite: {e}")
        return False
    
    # 连接MySQL
    mysql_conn = get_db_connection()
    if not mysql_conn:
        print("[迁移错误] 无法连接MySQL")
        sqlite_conn.close()
        return False
    ...
    try:
        for table in tables:
            ...
            mysql_conn.commit()
            print(f'[迁移] {table}: 成功迁移 {success_count}/{len(rows)} 条记录')
